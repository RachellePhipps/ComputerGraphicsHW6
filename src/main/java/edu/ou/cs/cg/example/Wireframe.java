	...
	implements GLEventListener, Runnable
{
	...
	{
		SwingUtilities.invokeLater(new Wireframe(args));
	}
	...
	private void	updatePipeline(GLAutoDrawable drawable)
	{
		...
		// v2: camera at X=Y=Z=3.0, looking near center of z-axis of cube
		...
		gl.glRotated((k / 5.0), (k / 3.0), (k / 2.0), 0);

		// v2: scale x, y, z so that cube pulses over time
		double	f = Math.cos(2.0 * Math.PI * (k % 120) / 120.0);
	...
	private void	drawText(GLAutoDrawable drawable)
	{
		...
		renderer.draw("Wireframe", 2, h - 14);
